...
const getXINRBooks= async function (req, res) {
    let priceData=await bookModel.find({"price.IndiaPrice":{$in:["Rs 100","Rs 200","Rs 200","Rs 500"]}})

    res.send({msg: priceData})
}

const getRandomBooks = async function (req, res) {
   let stockData=await bookModel.find({$or: [{stockAvilable:true},{totalpages:{$gt:500}}]});
